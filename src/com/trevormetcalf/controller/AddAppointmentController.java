...
import com.trevormetcalf.trevortaskc195.Validation;
import com.trevormetcalf.utility.Alerts;
import com.trevormetcalf.utility.AppointmentException;
...
/*
    This Controller correlates with the add appointment window.
    The primary goal is to take user input from FXML file,
    verify integrity of information, and then pass information to database.
    More detailed information can be found near each method.
 */
...
public class AddAppointmentController implements Initializable {
    ...
    String customerID, userID, title, description, location, contact, type;
    ...
    private void addAppointment() {
        String sql1 = "INSERT INTO appointment (customerId, userId, title, description, location, contact, type, start, end, createDate, createdBy, LastUpdateBy, url) "
                + "VALUES(" + customerID + ", " + userID + ", '" + title +"', '" + description + "', '"
                + location + "', '" + contact + "', '" + type + "', '" + df.format(start) + "', '" + df.format(end) + "', '" + df.format(TimeConverter.toUTC(LocalDateTime.now()))
    ...
    // Set location label based on selected customer's location
    public void updateLocationLabel() {
        if (customerTableView.getSelectionModel().getSelectedItem() != null) {
            locationField.setText(customerTableView.getSelectionModel().getSelectedItem().getCity());
        }
    }
