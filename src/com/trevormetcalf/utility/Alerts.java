...
public class Alerts {
    ...
    static ResourceBundle rb = ResourceBundle.getBundle("com/trevormetcalf/trevortaskc195/Nat", Locale.getDefault());


    public static Alert getAlert(String alertType) {
        // Check if Espanol and invalid alert
        if(Locale.getDefault().getLanguage().equals("es") && alertType.equals("invalidLogin")) {
            invalidLogin.setTitle(rb.getString("Invalid"));
            invalidLogin.setHeaderText(rb.getString("Check"));
            invalidLogin.setContentText(rb.getString("Try"));
            return invalidLogin;
        }
        switch(alertType) {
            case "invalidLogin":
